<!doctype html>
<x-header title="outgoing {{ $outgoing->product_name }}" />
                                ...
                                <div class="card h-100">
                                    <div class="card-header d-flex justify-content-between">
                                        <div class="card-title mb-0">
                                            <h5 class="mb-1 me-2">edit outgoing goods</h5>
                                        </div>

                                    </div>
                                    <div class="card-body  m-1">
                                        <form class=" row" enctype="multipart/form-data" action="/admin/outgoing/store"
                                            method="post">
                                            @csrf

                                            <div class=" col-lg-5 m-1 border rounded-2 p-1 ">
                                                <label for="product_id" class="form-label">product</label>
                                                <select onchange="productSelect(event)" value="{{ $outgoing->product_id}}" id="product_id"
                                                    name="product_id" class="form-select form-select-sm">
                                                    <option value="null" selected>--choose--</option>
                                                    @foreach ($products as $product)
                                                    @if ($product->id == $outgoing->product_id)
                                            ...
                                            <div class="form-floating col-lg-5 m-1">
                                                <input type="number" class="form-control" value="{{ $outgoing->quantity }}"
                                            ...
                                            <div class="form-floating col-lg-5 m-1">
                                                <input type="price" readonly class="form-control" value="{{ $outgoing->price }}"
                                            ...
                                            <div class=" col-lg-5 m-1 border rounded-2 p-1 ">
                                                ...
                                                <select value="{{ old('customer_id') }}" id="customer_id"
                                                    name="customer_id" class="form-select form-select-sm">
                                                    <option value="null" selected>--choose--</option>
                                                    @foreach ($customers as $customer)
                                                    @if ($customer->id == $outgoing->customer_id)
                                                    <option selected value="{{ $customer->id }}"> {{ $customer->name }}
                                                        <input style="opacity:0;color:white;" type="text" name="supplier_name" id="supplier_name" value="{{$customer->name  }}" />
                                                    </option>       
                                            ...
                                            <div class="form-floating col-lg-4 m-1">
                                                <input type="number" readonly class="form-control" value="{{ $outgoing->total }}"
